...
asthma <- read_rds("data/asthma_prevalence_2017_capcog.rds") |> 
    ...
    dplyr::select(GEOID, child_prev, adult_prev, total_prev, geometry) |>
    rename("Asthma Prevalence" = total_prev,
           "Asthma Prevalence in Children" = child_prev,
           "Asthma Prevalence in Adults" = adult_prev) |>
    pivot_longer(cols = `Asthma Prevalence in Children`:`Asthma Prevalence`,  
                 names_to = "var",
                 values_to = "value") 
...
test <- soc |>
  filter(var == "% White-Alone") |>
  distinct()

test <- as.data.frame(test)
    test <- st_as_sf(test)
    test <-
      st_transform(test, "+proj=longlat +ellps=WGS84 +datum=WGS84")


leaflet() %>% 
    addTiles() %>% 
    addPolygons(data=test, weight = 2, fillColor = "yellow")
