...
cap_ozone <- capcog_oz_raw |> 
    select(fid, fourth_hig, geometry) |> 
...
cap_pm2.5 <- capcog_pm2.5_raw |> 
    select(fid, b_pm25_ann, geometry) |> 
...
df$`Population Density` <-
    df$`Total population` / df$`Shape area` 
df$`Population Density`[df$`Population Density`== Inf] <- 0
df <- mutate(df, `Population Density` = `Population Density` * 27878400)
...
df <-
    ...
    select(
        ...
        "Percent of households without a car",
        "Population Density", #YOu need to add variables here if they are at the end
        everything()
    ) |>
    mutate(across(everything(), as.character)) |>
    pivot_longer(cols = 2:26, #This has to be adjusted when you add or remove variables 
                 names_to = "var",
                 values_to = "value") |>
